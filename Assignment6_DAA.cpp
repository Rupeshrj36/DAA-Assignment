...
// Function to increase utility of perishable items
void boostPerishableUtility(vector<Item>& items) {
    for (auto& it : items) {
        if (it.perishable) {
            it.utility = static_cast<int>(it.utility * 1.2); // 20% priority boost
        }
    }
}

// Function to print selected items from DP table
...
int main() {
    ...
    cout << "🚚 SwiftCargo - Truck Loading Optimization (Knapsack with Perishables)\n";
    ...
    // Apply perishable priority modifier
